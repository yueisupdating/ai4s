...
4. 同时生成《{{SINGLE_CHOICE_JSON}}》文件，保存单选题数字答案和多选题得分，不保存填空题、开放题、选项文字、证据、置信度或解释。JSON 必须是一个扁平对象，键必须正好是字符串题号 "2" 到 "46"。单选题值按 A=1、B=2、C=3、D=4 映射。多选题值按得分规则保存：选第一个选项记 1 分；未选第一个且只选其他 1 项记 2 分；未选第一个且选其他 2-4 项记 3 分；未选第一个且选其他 4 项以上记 4 分。例如 {"2": 3, "3": 1, "15": 4}；不要输出字母或字符串数字。

在进行任何文件读取或编辑之前，你必须先使用工具查看当前目录下的文件列表（如执行 ls 或类似技能），确认文件存在且名称无误后，再进行操作。注意最终问卷文件要包括原始问题和回答。多选问题所选的每个选项都要给出确切的证据，不得所有选项都选择并且泛泛给出依据。单选题答案 JSON 必须按选项顺序映射为数字：A=1、B=2、C=3、D=4。多选题 JSON 值必须是按上述规则计算后的得分，不要保存多选题所选选项列表。
